...
Nessa análise iremos observar o número de seguidores de cada parlamentar de cada partido para aí sim termos alguma conclusão se o número de representantes traz consigo a fama e oconhecimento na rede social.
...
ggplot() +
  geom_point(data = parla_psl,
             mapping = aes(x = nome_eleitoral, y = engaj_total_retweet, color = partido, size = seguidores)) +
  geom_point(data = parla_pode,
             mapping = aes(x = nome_eleitoral, y = engaj_total_retweet, color = partido, size = seguidores)) +
  scale_y_log10() + 
  labs(x = "Parlamentar",
       y = "Engajamento total por retweet",
       color = "Partido",
       size = "Seguidores")
```